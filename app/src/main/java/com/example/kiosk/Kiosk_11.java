            ...
            public void onInit(int status) {
                if (status != TextToSpeech.ERROR) {
                    if (getResources().getConfiguration().locale.getLanguage().equals("ko")) {
                        ...
                        speakText("마지막으로 결제하는 방법을 알아보겠습니다. 결제는 화면과 같이 키오스크 하단에 있는 네모칸에 카드를 3초간 접촉해 주시면 화면이 전환이 됩니다.");
                    } else {
...
//        }
//
//        ActivityCompat.requestPermissions(Kiosk_11.this, permission_list, 1); // 각자의 클래스명 지정해야합니다.
//
...
//        //여기까지
                                    ...
                                    handler.post(new Runnable() {
                                        ...
                                        public void run() {
                                            ...
                                            if (text.contains("success")) {
                                                Intent intent = new Intent(Kiosk_11.this, Kiosk_12.class); //(해당클래스명.this, 다음 클래스명.class)
                                                tts.shutdown();
                                                disconnectBluetooth();
    ...
    public void goto_kiosk_10(View view) {
        tts.shutdown();
